...
int main() {
  ...
  std::shared_ptr<torch::jit::script::Module> module =
      torch::jit::load("/home/t/Dropbox/gcn2.pt");
  // std::shared_ptr<torch::jit::script::Module> module =
  // torch::jit::load("/home/t/Dropbox/gcn_tiny.pt");
    ...
    if (index > 0) {
      ...
      auto img1_tensor =
          torch::CPU(torch::kFloat32)
              .tensorFromBlob(img1.data, {1, img_height, img_width, 1});
      img1_tensor = img1_tensor.permute({0, 3, 1, 2});
      auto img1_var =
          torch::autograd::make_variable(img1_tensor, false).to(device);
      ...
      cv::Mat descriptors;
      nms(pts_mat, desc_mat, keypoints, descriptors, border, dist_thresh,
